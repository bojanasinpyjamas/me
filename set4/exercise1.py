...
def get_some_details():
    ...
    d = data["results"][0]
    lastN = d["name"]["last"]
    passW = d["login"]["password"]
    postC = int(d["location"]["postcode"])
    iD = int(d["id"]["value"])

    return {"lastName": lastN, "password": passW, "postcodePlusID": postC + iD}
...
def pokedex(low=1, high=5):
    ...
    tallestpokemon = {}
    for p in gym:
        if p["height"] > tallestpokemon.get("height", 0):
            tallestpokemon = p
    Pokename = tallestpokemon["name"]
    weight = tallestpokemon["weight"]
    height = tallestpokemon["height"]
